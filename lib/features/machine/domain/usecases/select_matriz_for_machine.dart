...
class SelectMatrizForMachine implements UseCase<MachineConfig, SelectMatrizParams> {
  ...
  Future<Either<Failure, MachineConfig>> call(SelectMatrizParams params) async {
    ...
    return matrizResult.fold(
      (failure) => Left(failure),
      (matriz) async {
        if (!matriz.canBeUsed) {
          return Left(ValidationFailure(message: 'A matriz selecionada não está ativa ou não pode ser usada'));
        }
